  ...
  Prerequisites (10 minutes):
      ...
      - Verify the tables are starting to CDC to BigQuery (there will only be 1 row in each until you start the next DAG):
        ...
        SELECT * FROM `data-analytics-demo-itfo3j9qq5.datastream_private_ip_public.driver`;
        SELECT * FROM `data-analytics-demo-itfo3j9qq5.datastream_private_ip_public.review`;
        SELECT * FROM `data-analytics-demo-itfo3j9qq5.datastream_private_ip_public.payment`;
  ...
  SSH to the SQL Reverse Proxy (so you can run psql SQL statements against the Cloud SQL database via command line)
      - Open this: https://console.cloud.google.com/compute/instances?project=data-analytics-demo-itfo3j9qq5
      - Click on SSH
      ...
      - Open this:
        - Edit the Firewall rule: https://console.cloud.google.com/networking/firewalls/details/cloud-shell-ssh-firewall-rule?project=data-analytics-demo-itfo3j9qq5
        - Click Edit
...
SELECT COUNT(*) AS Cnt FROM `data-analytics-demo-itfo3j9qq5.datastream_private_ip_public.driver`;
SELECT COUNT(*) AS Cnt FROM `data-analytics-demo-itfo3j9qq5.datastream_private_ip_public.review`;
SELECT COUNT(*) AS Cnt FROM `data-analytics-demo-itfo3j9qq5.datastream_private_ip_public.payment`; 
...
(
    SELECT driver_id, driver_name
     FROM `data-analytics-demo-itfo3j9qq5.datastream_private_ip_public.driver`
)
, overall_drive_rating AS
(
    SELECT driver_id, AVG(CAST(review_rating AS FLOAT64)) AS average_rating
      FROM `data-analytics-demo-itfo3j9qq5.datastream_private_ip_public.review`
     GROUP BY 1
)
, warehouse_data AS
(
    SELECT trips.TaxiCompany, 
           ...
           reviews.driver_id
      FROM `data-analytics-demo-itfo3j9qq5.datastream_private_ip_public.review` AS reviews
           INNER JOIN `data-analytics-demo-itfo3j9qq5.taxi_dataset.taxi_trips` AS trips
...
SELECT driver.driver_name,
       ...
       trips.*
  FROM `data-analytics-demo-itfo3j9qq5.datastream_private_ip_public.payment` AS payment
       INNER JOIN `data-analytics-demo-itfo3j9qq5.taxi_dataset.taxi_trips` AS trips
       ...
       INNER JOIN `data-analytics-demo-itfo3j9qq5.datastream_private_ip_public.driver` AS driver
