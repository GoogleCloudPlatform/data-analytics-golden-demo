...
Clean up / Reset script:
    DROP VIEW IF EXISTS `${project_id}.${bigquery_taxi_dataset}.rideshare_trips`;
    DROP EXTERNAL TABLE IF EXISTS `${project_id}.${bigquery_taxi_dataset}.rideshare_trips_raw_parquet`;
    DROP EXTERNAL TABLE IF EXISTS `${project_id}.${bigquery_taxi_dataset}.rideshare_trips_manifest`;
...
CREATE OR REPLACE EXTERNAL TABLE `${project_id}.${bigquery_taxi_dataset}.rideshare_trips_raw_parquet`
...
uris = ['gs://${bucket_name}/delta_io/rideshare_trips/*.snappy.parquet']
);
...
hive_partition_uri_prefix = "gs://${bucket_name}/delta_io/rideshare_trips/_symlink_format_manifest/",
uris = ['gs://${bucket_name}/delta_io/rideshare_trips/_symlink_format_manifest/*']
);

-- Show our manifest files
-- This will only have files that are not logically deleted by Delta.IO
SELECT * from `${project_id}.${bigquery_taxi_dataset}.rideshare_trips_manifest`;
