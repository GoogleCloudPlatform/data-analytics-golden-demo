...
SELECT *
  FROM `${project_id}.${aws_omni_biglake_dataset_name}.taxi_s3_yellow_trips_csv_cls`
 WHERE year = 2021
   AND month = 1  
LIMIT 1000;
...
-- This will error since we try to select the Fare_Amount and Total_Amount
SELECT *
  FROM `${project_id}.${aws_omni_biglake_dataset_name}.taxi_s3_yellow_trips_json_cls`
 WHERE year = 2021
   AND month = 1  
LIMIT 1000;
...
-- This will error since we try to select the Fare_Amount and Total_Amount
SELECT *
  FROM `${project_id}.${aws_omni_biglake_dataset_name}.taxi_s3_yellow_trips_parquet_cls`
LIMIT 1000;
