...
spark.sql(f"DROP TABLE IF EXISTS {iceberg_catalog}.{iceberg_warehouse}.{iceberg_table}")
...
sql = f"CREATE TABLE IF NOT EXISTS {iceberg_catalog}.{iceberg_warehouse}.{iceberg_table} " + \
       ...
       "USING iceberg " + \
       f"TBLPROPERTIES(bq_table='{bq_dataset}.{iceberg_table}', bq_connection='{bq_region}.{biglake_connection}');"
