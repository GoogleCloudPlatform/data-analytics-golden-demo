...
# This will be the FINAL output schema for the DataEngineeringWorkflowSharedSessionAgent (the orchestrator)
...
# Wrapper for de_tools.check_data_quality_failures
async def check_for_data_quality_failures(data_quality_scan_name: str) -> dict:
    ...
    result = await asyncio.to_thread(
        de_tools.check_data_quality_failures,
...
# Wrapper for de_tools.get_data_quality_failure_analysis
async def get_failed_data(data_quality_scan_name: str, data_quality_scan_dataset_name: str, data_quality_scan_table_name: str) -> dict:
    ...
    result = await asyncio.to_thread(
        de_tools.get_data_quality_failure_analysis,
        data_quality_scan_name,
...
# Wrapper for de_tools.generate_data_engineering_fix_prompt
async def generate_data_enginnering_agent_correction_prompt(failed_rules_details: List[Dict[str, Any]]) -> dict:
    ...
    result = await asyncio.to_thread(de_tools.generate_data_engineering_fix_prompt, failed_rules_details)
...
# --- Orchestrator Class ---
class DataEngineeringWorkflowSharedSessionAgent(BaseAgent):
    """
    ...
    async def _run_async_impl(
        ...
        # Initial parameters are taken from the session state, as these are inputs to the orchestrator
        data_quality_scan_name_param = ctx.session.state.get("data_quality_scan_name_param", "")
        repository_name_param = ctx.session.state.get("repository_name_param", "")
        workspace_name_param = ctx.session.state.get("workspace_name_param", "")
        workflow_type_param = ctx.session.state.get("workflow_type_param", "")
        initial_query_param = ctx.session.state.get("initial_data_engineering_query_param", "")
