...
PROMPTS_BY_CATEGORY = {
    ...
    "AI Forecast": [
        {
            "prompt": "For The Daily Grind truck will we run out of any ingredients in the next 15 minutes?",
        ...
        {
            "prompt": "For truck 10 forecast the ingredients for the next 15 minutes.",
        ...
        {
            "prompt": "Forecast the number of people walking by the truck Mocha Mover for the next 15 minutes.",
        ...
        {
            "prompt": "Predict the grinder motor RPM for machine 1 for the next 15 minutes. Will it fall below a critical threshold of 1500?",
    ...
    "Data Engineering": [
        {
            "prompt": 'Create a pipeline named "Taxi Pipeline 01" that makes the fields (borough, zone and service_zone) all uppercase in the location table and saves it to a new table named "location_uppercase".',
...
async def get_prompt_categories() -> list[str]:
    """
...
async def get_prompts_for_category(category_name: str) -> str:
    """
    ...
    if not selected_category:
        available_categories = ", ".join(await get_prompt_categories())
