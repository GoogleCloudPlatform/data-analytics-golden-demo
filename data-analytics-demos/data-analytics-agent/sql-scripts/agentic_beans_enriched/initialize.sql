...
-- ** camera **
--------------------------------------------------------------------------------
CREATE OR REPLACE TABLE `agentic_beans_enriched.camera`
(
...
INSERT INTO `agentic_beans_enriched.camera`
...
SELECT camera_id, 
       ...
       installation_date,
       status
  FROM `agentic_beans_raw.camera`;


--------------------------------------------------------------------------------
-- ** customer **
--------------------------------------------------------------------------------
CREATE OR REPLACE TABLE `agentic_beans_enriched.customer`
(
...
INSERT INTO `agentic_beans_enriched.customer`
(customer_id, customer_name, customer_yob, customer_email, customer_inception_date, country_code)
WITH customer_inception AS
(
  SELECT customer_id,
         MIN(order_header_timestamp) AS customer_inception_date
    FROM `agentic_beans_raw.order_header`
   GROUP BY ALL
)
SELECT customer.customer_id,
       ...
       CAST(customer_inception.customer_inception_date AS DATE) AS customer_inception_date,
       customer.country_code
  FROM `agentic_beans_raw.customer` AS customer
       LEFT JOIN customer_inception
              ON customer.customer_id = customer_inception.customer_id;
...
-- ** event **
--------------------------------------------------------------------------------
CREATE OR REPLACE TABLE `agentic_beans_enriched.event`
(
...
INSERT INTO `agentic_beans_enriched.event`
...
SELECT event_id, 
       ...
       event_venue,
       event_neighborhood
  FROM `agentic_beans_raw.event`;


--------------------------------------------------------------------------------
-- ** ingredient **
--------------------------------------------------------------------------------
CREATE OR REPLACE TABLE `agentic_beans_enriched.ingredient`
(
...
INSERT INTO `agentic_beans_enriched.ingredient`
...
SELECT ingredient_id, 
       ...
       is_perishable
  FROM `agentic_beans_raw.ingredient`;


--------------------------------------------------------------------------------
-- ** machine **
--------------------------------------------------------------------------------
CREATE OR REPLACE TABLE `agentic_beans_enriched.machine`
(
...
INSERT INTO `agentic_beans_enriched.machine`
...
SELECT machine_id, 
       ...
       status
  FROM `agentic_beans_raw.machine`;


--------------------------------------------------------------------------------
-- ** order_detail **
--------------------------------------------------------------------------------
CREATE OR REPLACE TABLE `agentic_beans_enriched.order_detail` (
...
INSERT INTO `agentic_beans_enriched.order_detail` 
(order_detail_id, order_header_id, truck_menu_id, order_quantity, product_id, size, price)
SELECT order_detail.order_detail_id,
       order_detail.order_header_id,
       order_detail.truck_menu_id,
       order_detail.order_quantity,
       truck_menu.product_id,
       truck_menu.size,
       truck_menu.price,
  FROM `agentic_beans_raw.order_detail` AS order_detail
  INNER JOIN `agentic_beans_raw.truck_menu` AS truck_menu
...
-- ** order_header **
--------------------------------------------------------------------------------
CREATE OR REPLACE TABLE `agentic_beans_enriched.order_header` (
...
INSERT INTO  `agentic_beans_enriched.order_header`
(order_header_id, order_header_timestamp, truck_id, customer_id, order_neighborhood)
WITH truck_data AS
(
  -- We "should" only have 1 truck per order, but just in case row number is used
  SELECT order_detail.order_header_id, 
         truck_menu.truck_id,
         ROW_NUMBER() OVER (PARTITION BY order_detail.order_header_id) as ranking
    FROM `agentic_beans_raw.order_detail` AS order_detail
         INNER JOIN `agentic_beans_raw.truck_menu` AS truck_menu
                 ON order_detail.truck_menu_id = truck_menu.truck_menu_id
)
...
SELECT order_header.order_header_id,
       ...
       order_by_truck.truck_id,
       order_header.customer_id,
       order_header.order_neighborhood
       order_header_total
  FROM `agentic_beans_raw.order_header` AS order_header
       INNER JOIN order_by_truck
               ON order_header.order_header_id = order_by_truck.order_header_id;
...
-- ** product **
--------------------------------------------------------------------------------
CREATE OR REPLACE TABLE `agentic_beans_enriched.product`
(
...
INSERT INTO `agentic_beans_enriched.product`
(product_id, product_category_id, product_name, product_description, product_image_prompt, product_image_uri)
SELECT product_id, 
       product_category_id,
       product_name,
       product_description,
       ...
       product_image_uri
  FROM `agentic_beans_raw.product`;


--------------------------------------------------------------------------------
-- ** product_category **
--------------------------------------------------------------------------------
CREATE OR REPLACE TABLE `agentic_beans_enriched.product_category`
(
...
INSERT INTO `agentic_beans_enriched.product_category`
(product_category_id, product_category_name, product_category_description, product_category_image_prompt, product_category_image_uri)
...
--------------------------------------------------------------------------------
CREATE OR REPLACE TABLE `agentic_beans_enriched.telemetry_camera_vision`
(
...
OPTIONS(
    description="Enriched data from AI-powered camera analysis, tracking customer queue lengths and general foot traffic around coffee trucks."
);

INSERT INTO `agentic_beans_enriched.telemetry_camera_vision`
...
SELECT telemetry_camera_vision_id, 
  ...
  FROM `agentic_beans_raw.telemetry_camera_vision`
WHERE telemetry_timestamp >= TIMESTAMP_SUB(CURRENT_TIMESTAMP(), INTERVAL 730 DAY); -- 2 years

--------------------------------------------------------------------------------
-- ** telemetry_coffee_machine **
--------------------------------------------------------------------------------
CREATE OR REPLACE TABLE `agentic_beans_enriched.telemetry_coffee_machine`
(
...
OPTIONS(
    description="Enriched telemetry data from coffee machines, used for real-time health monitoring, anomaly detection, and predictive maintenance insights."
);

INSERT INTO `agentic_beans_enriched.telemetry_coffee_machine`
...
-- ** telemetry_inventory **
--------------------------------------------------------------------------------
CREATE OR REPLACE TABLE `agentic_beans_enriched.telemetry_inventory`
(
...
CLUSTER BY truck_id, telemetry_timestamp
OPTIONS(
    description="Enriched inventory level updates for all consumables on coffee trucks, used for real-time stock management and replenishment planning."
);

INSERT INTO `agentic_beans_enriched.telemetry_inventory`
...
SELECT telemetry_inventory_id, 
       ...
       source_sensor_id,
  FROM `agentic_beans_raw.telemetry_inventory`
WHERE telemetry_timestamp >= TIMESTAMP_SUB(CURRENT_TIMESTAMP(), INTERVAL 730 DAY); -- 2 years

--------------------------------------------------------------------------------
-- ** truck **
--------------------------------------------------------------------------------
CREATE OR REPLACE TABLE `agentic_beans_enriched.truck`
(
...
INSERT INTO `agentic_beans_enriched.truck`
...
SELECT truck_id, 
  ...
  FROM `agentic_beans_raw.truck`;


--------------------------------------------------------------------------------
-- ** truck_menu **
--------------------------------------------------------------------------------
CREATE OR REPLACE TABLE `agentic_beans_enriched.truck_menu`
(
...
INSERT INTO `agentic_beans_enriched.truck_menu`
...
SELECT truck_menu_id,
       ...
       size, 
       price
  FROM `agentic_beans_raw.truck_menu`;


--------------------------------------------------------------------------------
-- ** weather **
--------------------------------------------------------------------------------
CREATE OR REPLACE TABLE `agentic_beans_enriched.weather` (
...
INSERT INTO `agentic_beans_enriched.weather`
...
SELECT weather_id, 
       ...
       is_day,
       data_source
  FROM `agentic_beans_raw.weather`;
